    ...
    def walk(value: Any) -> None:
        if isinstance(value, dict):
            for key, child in value.items():
                if key in buckets:
                    if isinstance(child, str) and child.startswith("http") and child not in buckets[key]:
...
def download_one(context, item: dict[str, Any]) -> dict[str, Any]:
    ...
    result: dict[str, Any] = {
        ...
        "signed_url_count": len(media_urls),
        "attempts": [],
    }
        ...
        try:
            with part.open("wb") as handle:
                while total is None or offset < total:
                    ...
                    body = response.body()
                    first_type = first_type or response.headers.get("content-type")
                    if response.status not in (200, 206):
                        raise RuntimeError(f"HTTP {response.status} at {offset}")
                    if not body:
                        raise RuntimeError(f"empty response at {offset}")
                    if offset == 0 and not (
                        (first_type or "").lower().startswith("video/") or b"ftyp" in body[:64]
                    ):
                        raise RuntimeError(f"not mp4: {first_type}")
            ...
            if proc.returncode == 0:
                ffprobe = {"ok": True, "data": json.loads(proc.stdout)}
            else:
                ffprobe = {"ok": False, "stderr": proc.stderr[-2000:]}
...
def run() -> dict[str, Any]:
    report: dict[str, Any] = {
        "probe": "tiktok-e2e-discovery-download-v1",
        "tag": TAG,
        "started_at": now_iso(),
    }
    ...
    with sync_playwright() as p:
        ...
        top = sorted(rows, key=lambda x: x.get("play_count") or 0, reverse=True)[:DOWNLOAD_COUNT]
        downloads = [download_one(context, item) for item in top]
        context.close()
        browser.close()
    ...
    report.update({
        ...
        "selected": top,
        "downloads": downloads,
        "successful_downloads": sum(1 for row in downloads if row.get("ok")),
        "finished_at": now_iso(),
    })
    return report
...
def main() -> int:
    ...
    summary = {
        ...
        "downloads": [
            {k: row.get(k) for k in ("id", "ok", "size_bytes", "sha256", "chunks", "content_type", "ffprobe", "attempts")}
            for row in report["downloads"]
        ],
    }
    dump("e2e_summary.json", summary)
    print("TIKTOK_E2E_SUMMARY=" + json.dumps(summary, ensure_ascii=False))

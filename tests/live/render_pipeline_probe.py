...
run("ffmpeg", "-hide_banner", "-loglevel", "error", "-y", "-ss", "0.2", "-i", str(ROOT/"a.mp4"), "-t", "1.2",
    ...
    "-c:v", "libx264", "-preset", "veryfast", "-crf", "28", "-c:a", "aac", "-b:a", "96k", "-ar", "48000", "-ac", "2", "-shortest", str(ROOT/"seg1.mp4"))
...
run("ffmpeg", "-hide_banner", "-loglevel", "error", "-y", "-loop", "1", "-framerate", "24", "-i", str(ROOT/"freeze.jpg"), "-i", str(ROOT/"comment.mp3"), "-t", "0.8",
    ...
    "-c:v", "libx264", "-preset", "veryfast", "-crf", "28", "-c:a", "aac", "-b:a", "96k", "-ar", "48000", "-ac", "2", "-shortest", str(ROOT/"seg2.mp4"))
...
run("ffmpeg", "-hide_banner", "-loglevel", "error", "-y", "-ss", "0.5", "-i", str(ROOT/"b.mp4"), "-f", "lavfi", "-i", "anullsrc=r=48000:cl=stereo", "-t", "1.0",
    ...
    "-c:v", "libx264", "-preset", "veryfast", "-crf", "28", "-c:a", "aac", "-b:a", "96k", "-ar", "48000", "-ac", "2", "-shortest", str(ROOT/"seg3.mp4"))
...
run("ffmpeg", "-hide_banner", "-loglevel", "error", "-y", "-f", "concat", "-safe", "0", "-i", str(listing), "-c", "copy", "-movflags", "+faststart", str(ROOT/"mix.mp4"))

info = probe_json(ROOT/"mix.mp4")
...
summary = {
    ...
    "strategy": "sequential normalize -> cached-ready segments -> concat stream copy",
}
